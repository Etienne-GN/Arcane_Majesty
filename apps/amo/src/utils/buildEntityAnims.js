...
export function buildEntityAnims(anims, animPrefix, profileKey = 'rpgmaker_32', textureKey = null) {
    ...
    // Supports both a plain string texture key and a { up, down, left, right } map per animation type.
    const txFor = (type, dir = null) => {
        const entry = p.textures?.[type] ?? defaultTx;
        if (dir && typeof entry === 'object' && entry !== null) return entry[dir] ?? defaultTx;
        if (typeof entry === 'object' && entry !== null) return Object.values(entry)[0] ?? defaultTx;
        return entry;
    };
    ...
    if (p.attack && !Array.isArray(p.attack)) {
        for (const dir of ['down', 'left', 'right', 'up']) {
            if (p.attack[dir]) {
                anims.create({
                    ...
                    frames: makeFrames(txFor('attack', dir), p.attack[dir]),
                    frameRate: fr.attack,
                    repeat: 0,
                });
            }
        ...
        if (p.attack.down) {
            anims.create({
                key: `${prefix}attack`,
                frames: makeFrames(txFor('attack', 'down'), p.attack.down),
                frameRate: fr.attack,
                repeat: 0,
            });
        }
    ...
    // Directional graze (optional — farm/passive animals with eat animation sheets)
    if (p.graze) {
        for (const dir of ['down', 'left', 'right', 'up']) {
            if (p.graze[dir]) {
                anims.create({
                    key: `${prefix}graze_${dir}`,
                    frames: makeFrames(txFor('graze', dir), p.graze[dir]),
                    frameRate: fr.graze ?? 4,
                    repeat: -1,
                });
            }
        }
