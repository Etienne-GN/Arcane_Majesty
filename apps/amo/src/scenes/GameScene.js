...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        playerStats.onSpellEvent((id, level, isDiscovery) => {
            ...
            const verb = isDiscovery ? 'Learned' : 'Upgraded';
            ui?.showNotification?.(`${verb}: ${spell.name} — ${tierName}`, 3500);
        ...
        // Chapter title card — skip in online mode (lore is Eldrin-specific)
        if (mapDef.chapterTitle && !this._serverUrl) this._showChapterTitle(mapDef.chapterTitle);

        // Intro dialogue on first visit — skip in online mode
        if (mapDef.introDialogue && mapDef.introRegistryKey && !this.registry.get(mapDef.introRegistryKey) && !this._serverUrl) {
    ...
    _connectNetwork(px, py) {
        const name = localStorage.getItem('amo_name') ?? this._genPlayerName();
        localStorage.setItem('amo_name', name);

        networkManager.connect(this._serverUrl);
        // join() queues the payload and sends it the moment the socket connects
        networkManager.join(name, this._characterId, px, py, 'down', this._mapId);

        networkManager.on('init',   (others) => others.forEach(s => this._addRemotePlayer(s)));
        networkManager.on('joined', (state)  => this._addRemotePlayer(state));
        networkManager.on('moved',  ({ id, x, y, facing }) => this._remotePlayers.get(id)?.applyState(x, y, facing));
        networkManager.on('left',   ({ id }) => {
            this._remotePlayers.get(id)?.destroy();
        ...
        networkManager.on('enemySync', (states) => {
            states.forEach(({ id, x, y, health, facing }) => {
                ...
                if (e?.active) e.applyNetState(x, y, health, facing);
            });
        });
    ...
    _addRemotePlayer(state) {
        if (this._remotePlayers.has(state.id)) return;
        const rp = new RemotePlayer(this, state);
        this._remotePlayers.set(state.id, rp);
    }

    _genPlayerName() {
        const adj  = ['Arcane', 'Void', 'Storm', 'Silver', 'Shadow'];
        const noun = ['Walker', 'Seeker', 'Mage', 'Scout', 'Sage'];
