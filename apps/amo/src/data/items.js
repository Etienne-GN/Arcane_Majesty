...
export const ITEMS = {
    ...
    // ── Wildlife drops ────────────────────────────────────────────────────────
    turkey_meat: {
        id: 'turkey_meat', name: 'Turkey Meat',
        description: 'Plump and savoury. Restores 12 HP.',
        color: 0xcc8855, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 5,
        onUse: (stats) => { stats.health = Math.min(stats.health + 12, stats.maxHealth); return true; }
    },
    boar_meat: {
    ...
    herb_crusted_rabbit: {
        id: 'herb_crusted_rabbit', name: 'Herb-Crusted Rabbit',
        description: 'Rabbit rolled in forest herbs and mushroom dust. Restores 30 HP, 15 MP, and regenerates 20 HP.',
        color: 0xbbcc88, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 12,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 30, stats.maxHealth);
    ...
    wild_herb_broth: {
        id: 'wild_herb_broth', name: 'Wild Herb Broth',
        description: 'Thin venison broth steeped with herbs. Restores 12 HP and 25 MP.',
        color: 0x99cc77, icon: 'itm_herb', stackable: true, buyPrice: 0, sellPrice: 9,
        onUse: (stats) => {
    ...
    herb_mushroom_pie: {
        id: 'herb_mushroom_pie', name: 'Herb & Mushroom Pie',
        description: 'A hearty forest pie. Restores 35 HP, 25 MP, and regenerates 20 HP over time.',
        color: 0xcc9955, icon: 'itm_sandwich', stackable: true, buyPrice: 0, sellPrice: 16,
        onUse: (stats) => {
    ...
    lucky_rabbit_stew: {
        id: 'lucky_rabbit_stew', name: "Lucky Rabbit's Stew",
        description: "Brewed with a rabbit's foot. Restores 22 HP and applies Blessed for 20s.",
        color: 0xddcc88, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 14,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 22, stats.maxHealth);
            stats._applyStatus?.('blessed', { duration: 20000 });
            return true;
        }
    },
    frost_venison: {
        id: 'frost_venison', name: 'Frost-Cured Venison',
        description: 'Venison slowly cured in enchanted ice. Restores 35 HP and regenerates 28 HP over time.',
        color: 0xaaddee, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 13,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 35, stats.maxHealth);
            stats._foodRegen = (stats._foodRegen ?? 0) + 28;
            return true;
        }
    },
    double_boar_rack: {
        id: 'double_boar_rack', name: 'Double Boar Rack',
        description: 'Two whole boar ribs roasted to perfection. Restores 65 HP instantly.',
        color: 0xdd7733, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 20,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 65, stats.maxHealth);
            return true;
        }
    },
    char_venison: {
        id: 'char_venison', name: 'Char-Seared Venison',
        description: 'Venison blackened over fierce ember heat. Restores 60 HP instantly.',
        color: 0x663322, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 18,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 60, stats.maxHealth);
            return true;
        }
    ...
    ice_smoked_boar: {
        id: 'ice_smoked_boar', name: 'Ice-Smoked Boar',
        description: 'Boar slow-smoked over crystalline ice vapour. Restores 50 HP and regenerates 30 HP.',
        color: 0xaaccdd, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 17,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 50, stats.maxHealth);
            stats._foodRegen = (stats._foodRegen ?? 0) + 30;
            return true;
    ...
    boar_mushroom_stew: {
        id: 'boar_mushroom_stew', name: 'Boar & Mushroom Stew',
        description: 'Boar simmered with rich mushrooms. Restores 50 HP and regenerates 40 HP over time.',
        color: 0xaa6633, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 18,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 50, stats.maxHealth);
            stats._foodRegen = (stats._foodRegen ?? 0) + 40;
            return true;
    ...
    mushroom_risotto: {
        ...
        description: 'Dense spore risotto with venison bits. Restores 22 HP, 40 MP, and boosts mana regen for 15s.',
        color: 0xbbaa66, icon: 'itm_mushroom', stackable: true, buyPrice: 0, sellPrice: 15,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 22, stats.maxHealth);
    ...
    venison_tartare: {
        id: 'venison_tartare', name: 'Venison Tartare',
        description: 'Finely prepared raw venison. Restores 20 HP and 20 MP instantly.',
        color: 0xcc5544, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 10,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 20, stats.maxHealth);
            stats.mana   = Math.min(stats.mana   + 20, stats.maxMana);
            return true;
    ...
    spectral_broth: {
        id: 'spectral_broth', name: 'Spectral Broth',
        description: 'Venison simmered with spectral dust. Restores 25 HP and 35 MP, and triples mana regen for 8s.',
        color: 0xbbaaff, icon: 'itm_sandwich', stackable: true, buyPrice: 0, sellPrice: 17,
        onUse: (stats) => {
    ...
    void_jerky: {
        id: 'void_jerky', name: 'Void Jerky',
        description: 'Rabbit dried in void brine. Restores 40 HP but drains 10 MP. Unsettling aftertaste.',
        color: 0x882299, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 11,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 40, stats.maxHealth);
    ...
    corrupted_stew: {
        id: 'corrupted_stew', name: 'Corrupted Stew',
        description: 'Boar cooked in corrupted essence. Restores 90 HP but drains 30 MP. Power comes at a price.',
        color: 0x660088, icon: 'itm_meat', stackable: true, buyPrice: 0, sellPrice: 19,
        onUse: (stats) => {
            stats.health = Math.min(stats.health + 90, stats.maxHealth);
            stats.mana   = Math.max(0, stats.mana - 30);
            return true;
        }
    },
    bone_herb_soup: {
...
export const COOKING_RECIPES = [
    ...
    // ── Herb combos ───────────────────────────────────────────────────────────
    { output: 'herb_crusted_rabbit', label: 'Herb-Crusted Rabbit',  ingredients: [{ id: 'rabbit_meat',  qty: 1 }, { id: 'forest_herb',    qty: 2 }, { id: 'mushroom_spore', qty: 1 }] },
    { output: 'wild_herb_broth',     label: 'Wild Herb Broth',      ingredients: [{ id: 'deer_meat',    qty: 1 }, { id: 'forest_herb',    qty: 3 }] },
    { output: 'herb_mushroom_pie',   label: 'Herb & Mushroom Pie',  ingredients: [{ id: 'deer_meat',    qty: 1 }, { id: 'forest_herb',    qty: 2 }, { id: 'mushroom_spore', qty: 2 }] },
    { output: 'lucky_rabbit_stew',   label: "Lucky Rabbit's Stew",  ingredients: [{ id: 'rabbit_meat',  qty: 1 }, { id: 'rabbit_foot',   qty: 1 }, { id: 'forest_herb',    qty: 1 }] },
    // ── Elemental roasts ──────────────────────────────────────────────────────
    { output: 'frost_venison',       label: 'Frost-Cured Venison',  ingredients: [{ id: 'deer_meat',    qty: 1 }, { id: 'ice_crystal',   qty: 1 }] },
    { output: 'double_boar_rack',    label: 'Double Boar Rack',     ingredients: [{ id: 'boar_meat',    qty: 2 }] },
    { output: 'char_venison',        label: 'Char-Seared Venison',  ingredients: [{ id: 'deer_meat',    qty: 2 }, { id: 'ember_stone',   qty: 1 }] },
    ...
    // ── Exotic & dangerous ────────────────────────────────────────────────────
    { output: 'void_jerky',          label: 'Void Jerky',           ingredients: [{ id: 'rabbit_meat',  qty: 1 }, { id: 'void_shard',    qty: 1 }] },
    { output: 'corrupted_stew',      label: 'Corrupted Stew',       ingredients: [{ id: 'boar_meat',    qty: 1 }, { id: 'corrupted_essence', qty: 1 }] },
    { output: 'bone_herb_soup',      label: 'Bone & Herb Soup',     ingredients: [{ id: 'bone_fragment', qty: 2 }, { id: 'forest_herb',  qty: 2 }] },
    { input: ['boar_meat', 'deer_meat', 'rabbit_meat', 'mushroom_spore'], output: 'grand_feast', label: 'Grand Feast', multi: true },
];
